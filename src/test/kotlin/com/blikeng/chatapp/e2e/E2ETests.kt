...
class E2ETests : ContainerBase() {
    ...
    fun shouldInviteAndJoinRoom() {
        mockMvc
            .post("/api/invites/room") {
                contentType = MediaType.APPLICATION_JSON
                content =
                    """{"type":"ROOM_INVITE","targetUsername":"username2","roomId":"$roomId","expiresAt":${System.currentTimeMillis() + 604800000}}"""
                user1Cookie?.let { cookie(it) }
            }.andExpect { status { isOk() } }
    ...
    fun shouldGetRoomHistory() {
        mockMvc
            .get("/api/chats/$roomId?page=0&size=25") {
                user2Cookie?.let { cookie(it) }
            }.andExpect { status { isOk() } }
            .andExpect {
                content {
    ...
    fun shouldMakeEncryptedRoom() {
        ...
        encryptedRoomId =
            json
                .first { it["encrypted"].asBoolean() }["roomId"]
                .asText()
    }
    ...
    fun shouldGetEncryptedRoomHistory() {
        mockMvc
            .get("/api/chats/$encryptedRoomId?page=0&size=25") {
                user2Cookie?.let { cookie(it) }
            }.andExpect { status { isOk() } }
            .andExpect {
    ...
    fun shouldCreateOpenRoomInvite() {
        ...
        val openInviteRoomId =
            objectMapper
                .readTree(roomsResult.response.contentAsString)
                .first { it["roomName"].asText() == "Open invite room" }["roomId"]
                .asText()
