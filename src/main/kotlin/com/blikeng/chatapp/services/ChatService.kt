...
import com.blikeng.chatapp.dtos.websocket.chat.WsMessageNotification
...
class ChatService(
    ...
    fun joinRoom(
        ...
        synchronized(session) {
            if (session.isOpen) {
                val users =
                    getUsersInRoom(roomId).map { member ->
                        RoomMember(
                            id = member.id,
                        ...
                        SendMessageDTO(
                            ...
                            message =
                                encrypt.decrypt(
                                    message.ciphertext,
                                    message.nonce,
                                    aad = configureAad(roomId, message.id, message.userId),
                                ),
